...
exports.createProductValidator = [
  check('title')
    ...
    .custom((title) => 
    ProductModel.findOne({title: title}).then((Title) => {
     if (Title){
  ...
  check('subcategories')
    ...
    SubCategoryModel.find({category : req.body.category}).then((categories) => {
      const subCategoriesIdsInDB = [];
      categories.forEach((subcategory) => {
        subCategoriesIdsInDB.push(subcategory._id.toString())
      })
      // check if subcategories ids in db include subcategories in req.body (true)
      const checker = (target , arr) => target.every((subcategory) => arr.includes(subcategory) )
...
exports.updateProductValidator = [
  ...
  validatorMiddleWare ,
  check("title").notEmpty().withMessage("product title can't be empty")
  .optional()
